...
function readURLAndSendAjax(input) {
    ...
    if (!checkSize) {
        $(input).closest('.form-group').find('.invalid-feedback').text("File không được quá 10MB").show();
        return false;
    }
    // if (!checkType) {
    //     $(input).closest('.form-group').find('.invalid-feedback').text("Đuôi ảnh chọn là JPG,PNG").show();
    //     return false;
    // }
...
function checkFileSize(fileSize, input, max = 10) {
    if (fileSize > max) {
        return false;
    }
    return true;
